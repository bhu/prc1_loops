library(data.table)
library(tidyverse)
library(readxl)
library(patchwork)
library(ggh4x)
library(tximport)
library(rtracklayer)
library(eulerr)
library(RcppCNPy)
library(pals)
library(scattermore)
library(InteractionSet)
library(ggpubr)
library(gggenes)
library(ggnewscale)

tclrs <- c('#e45756', '#4c78a8')

read_excel('../data/quant/mESC.ms.xlsx') %>%
  {.[,colSums(is.na(.)) != nrow(.)]} %>% 
  na.omit() %>% 
  {.[,c(1, which(.[1,] == 'Area'))]} %>%
  tail(-1) %>%
  dplyr::rename(p = 1) %>%
  `names<-`(., sub('\\..*', '', names(.))) %>%
  separate(p, c('pep', 'mod'), '\\ ') %>%
  #mutate(pep = sub('H33', 'H3', pep)) %>%
  group_by(pep) %>%
  mutate(across(-mod, function(x) as.numeric(x) / sum(as.numeric(x)))) %>%
  ungroup() %>% 
  filter(grepl('^H[34]', pep) & mod != 'unmod') %>%
  mutate(mod = strsplit(mod, '(?<=.)(?=K)', perl = T)) %>% 
  unnest(mod) %>%
  mutate(mod = paste(sub('_.*', '', pep), mod)) %>%
  dplyr::select(-pep) %>%
  group_by(mod) %>%
  summarise(across(everything(), sum), .groups = 'drop') %>%
  pivot_longer(-mod, names_to = 'samp', values_to = 'a') %>%
  mutate(type = sub('-2$', '', samp)) %>%
  filter(grepl('H3 K27|H3 K36', mod)) %>%
  dplyr::select(samp, mod, a) %>%
  filter(grepl('A1_par|NSD1', samp)) %>%
  filter(mod %in% c('H3 K27me3', 'H3 K36me2')) %>%
  mutate(geno = ifelse(grepl('NSD1', samp), 'NSD1-KO', 'WT') %>%
           factor(c('WT','NSD1-KO')),
         mod = sub('H3 ', '', mod)) %>%
  ggplot(aes(x = geno, y = a * 100, fill = geno)) +
  stat_summary(geom = 'errorbar', fun.data = mean_se, width = .25) +
  stat_summary(geom = 'col', fun = mean) +
  facet_nested(.~'mESC' + mod) +
  scale_fill_manual(values = tclrs) +
  scale_y_continuous('Abundance (%)', expand = expansion(c(0, .05))) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1),
        axis.text = element_text(size = 11, color = 'black'),
        panel.background = element_rect(fill = NA, color = 'black', size = 1),
        panel.grid = element_blank(),
        plot.background = element_blank(),
        strip.background = element_rect(fill = 'black'),
        strip.text = element_text(size = 11, color = 'white'),
        legend.position = 'none',
        axis.title.x = element_blank(),
        axis.title.y = element_blank(),
        panel.grid.major = element_line(color = 'grey80', linetype = 'dashed')) -> p2


read_excel('../data/quant/NCRM1.ms.xlsx') %>%
  {.[,colSums(is.na(.)) != nrow(.)]} %>% 
  na.omit() %>% 
  {.[,c(1, which(.[1,] == 'Area'))]} %>%
  tail(-1) %>%
  dplyr::rename(p = 1) %>%
  `names<-`(., sub('\\..*', '', names(.))) %>%
  separate(p, c('pep', 'mod'), '\\ ') %>%
  mutate(pep = sub('H33', 'H3', pep)) %>%
  group_by(pep) %>%
  mutate(across(-mod, function(x) as.numeric(x) / sum(as.numeric(x)))) %>%
  ungroup() %>% 
  filter(grepl('^H[34]', pep) & mod != 'unmod') %>%
  mutate(mod = strsplit(mod, '(?<=.)(?=[SK])', perl = T)) %>% 
  unnest(mod) %>%
  mutate(mod = paste(sub('_.*', '', pep), mod)) %>%
  dplyr::select(-pep) %>%
  group_by(mod) %>%
  summarise(across(everything(), sum), .groups = 'drop') %>%
  pivot_longer(-mod, names_to = 'samp', values_to = 'a') %>%
  mutate(samp = sub('^[0-9]*', '', samp)) %>%
  filter(grepl('K27|K36', mod)) %>%
  dplyr::select(samp, mod, a) %>%
  separate(mod, c('pep', 'mod')) -> d_2

read_excel('../data/quant/NCRM1.ms.xlsx') %>%
  {.[,colSums(is.na(.)) != nrow(.)]} %>% 
  na.omit() %>% 
  {.[,c(1, which(.[1,] == 'Area'))]} %>%
  tail(-1) %>%
  dplyr::rename(p = 1) %>%
  `names<-`(., sub('\\..*', '', names(.))) %>%
  separate(p, c('pep', 'mod'), '\\ ') %>%
  group_by(pep) %>%
  mutate(across(-mod, function(x) as.numeric(x) / sum(as.numeric(x)))) %>%
  ungroup() %>% 
  filter(grepl('^H[34]', pep) & mod != 'unmod') %>%
  mutate(mod = strsplit(mod, '(?<=.)(?=[SK])', perl = T)) %>% 
  unnest(mod) %>%
  mutate(mod = paste(sub('_.*', '', pep), mod)) %>%
  dplyr::select(-pep) %>%
  group_by(mod) %>%
  summarise(across(everything(), sum), .groups = 'drop') %>%
  pivot_longer(-mod, names_to = 'samp', values_to = 'a') %>%
  mutate(samp = sub('^[0-9]*', '', samp)) %>%
  filter(grepl('K27|K36', mod)) %>%
  dplyr::select(samp, mod, a) %>%
  separate(mod, c('pep', 'mod')) %>%
  mutate(pep = ifelse(pep == 'H3', 'H3.1/2', pep)) %>%
  rbind(d_2) %>%
  mutate(pep = ifelse(pep == 'H3', 'H3 (weighted average)', pep),
         mod = factor(mod, c(paste0('K27me', 1:3), paste0('K36me', 1:3), 'K27ac')),
         geno = ifelse(grepl('cN3[68]', samp), 'NSD1+/-', 'WT') %>%
           factor(c('WT','NSD1+/-'))) %>%
  filter(pep == 'H3.1/2') %>%
  filter(mod %in% c('K27me3','K36me2')) %>%
  mutate(mod = paste0('H3', mod)) %>%
  filter(grepl('cE2|cN', samp)) %>%
  ggplot(aes(x = geno, y = a * 100, fill = geno)) +
  stat_summary(geom = 'errorbar', fun.data = mean_se, width = .25) +
  stat_summary(geom = 'col', fun = mean) +
  facet_nested(.~'hiPSC' + mod) +
  scale_fill_manual(values = tclrs) +
  scale_y_continuous('Abundance (%)', expand = expansion(c(0, .05))) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1),
        axis.text = element_text(size = 11, color = 'black'),
        panel.background = element_rect(fill = NA, color = 'black', size = 1),
        panel.grid = element_blank(),
        plot.background = element_blank(),
        strip.background = element_rect(fill = 'black'),
        strip.text = element_text(size = 11, color = 'white'),
        legend.position = 'none',
        axis.title.x = element_blank(),
        axis.title.y = element_blank(),
        panel.grid.major = element_line(color = 'grey80', linetype = 'dashed')) -> p3
p3

{wrap_plots(p2,p3,nrow=1) &
    theme(plot.background = element_blank())} %>%
  ggsave('sf7_a.pdf', ., height = 3.1, width = 5.6, bg = 'transparent')


fs <- list.files( '../data/rna/ncrm1', full.names = T, pattern = 'sf') %>%
  setNames(., sub('.sf', '', basename(.)))
g <- import.gff3('../../pp_rna/gencode.v36.annotation.gff3.gz')
tx2gene <- g[g$type == 'transcript'] %>% as_tibble() %>%
  select(transcript_id, gene_id) %>% distinct()
txi <- tximport(fs, 'salmon', tx2gene = tx2gene)

g[g$type == 'gene' & g$gene_name == 'NSD1']$gene_id %>%
  txi$abundance[.,] %>%
  data.frame(v = .) %>%
  rownames_to_column('s') %>%
  mutate(type = ifelse(grepl('NPC', s), 'NPC', 'iPSC'),
         cond = ifelse(grepl('cN3', s), 'NSD1+/-','WT') %>%
           factor(c('WT', 'NSD1+/-'))) %>%
  ggplot(aes(x = cond, y = v, color = cond, fill = cond)) +
  geom_boxplot() +
  stat_summary(geom = "crossbar", width=0.6, fatten=0, color="white", 
               fun.data = function(x){ return(c(y=median(x), ymin=median(x), ymax=median(x))) }) +
  facet_wrap(~type) +
  scale_color_manual(values = tclrs) +
  scale_fill_manual(values = tclrs) +
  scale_y_log10('NSD1 TPM') +
  theme(panel.grid = element_blank(),
        legend.position = 'none',
        panel.background = element_rect(fill = NA, color = 'black', size = 1),
        strip.background = element_rect(fill = 'black'),
        strip.text = element_text(color = 'white', size = 11),
        axis.text = element_text(color = 'black', size = 11),
        plot.background = element_blank(),
        axis.title.x = element_blank(),
        panel.grid.major = element_line(color = 'grey80', linetype = 'dashed'),
        axis.text.x = element_text(angle = 45, hjust = 1)) -> p

ggsave('sf7_b.pdf', p, height = 3, width = 1.9, bg = 'transparent')

cgis <- list.files('../data/cgi', full.names = T) %>%
  setNames(., sub('.bed', '', basename(.))) %>%
  lapply(import)



getWhisks <- function(x) {
  x <- as.numeric(x)
  qs <- quantile(x, c(0.25, 0.75), na.rm = T)
  data.frame(lower = qs[1], upper = qs[2], middle = median(x, na.rm = T),
             ymin = min(x[x >= (qs[1] - 1.5 * diff(qs))], na.rm = T),
             ymax = max(x[x <= (qs[2] + 1.5 * diff(qs))], na.rm = T)) %>%
    mutate(notchlower = middle - 1.58 * diff(qs)/sqrt(length(x)),
           notchupper = middle + 1.58 * diff(qs)/sqrt(length(x)))
}


pks <- readRDS('../data/pks/PRC.rds')
clrs <- c('#f28e2b', '#59a14f', '#b07aa1', '#9c755f')

list(hiPSC = list(fs = c(`WT` = 'hiPSC_WT',
                         `NSD1+/-` = 'hiPSC_NSD1het'),
                  nm = 'hiPSC',
                  gn = 'hg38')) %>%
  Map(function(i, proj) {
    print(proj)
    dd <- lapply(i$fs, function(x) fread(sprintf('../data/prc_csight/%s.tsv', x))) %>%
      {merge(.[[1]], .[[2]], 
             by = c('chrom1', 'start1', 'end1', 'chrom2', 'start2', 'end2'))} %>%
      mutate(d = score.x - score.y) %>%
      filter(is.finite(d))
    
    ii <- list(dd[,1:3],  dd[,4:6]) %>%
      lapply(function(y) {
        y %>%
          `colnames<-`(c('chr', 'start', 'end')) %>%
          makeGRangesFromDataFrame()
      }) %>%
      {GInteractions(.[[1]], .[[2]], mode = 'strict')}
    
    u <- GRangesList(pks[[proj]]) %>% unlist() %>% reduce() %>% subsetByOverlaps(cgis[[i$gn]])
    p <- lapply(pks[[proj]], function(x) overlapsAny(u, x))
    
    vv <- list(All_three = p$CBX2 & p$RING1B & p$H3K27me3,
               Ring1B_only = p$RING1B & (!p$CBX2) &  (!p$H3K27me3),
               #CBX2_only = p$CBX2 & (!p$RING1B) &  (!p$H3K27me3),
               H3K27me3_only = p$H3K27me3 & (!p$CBX2) &  (!p$RING1B)) %>%
      lapply(function(x) {
        u[x] %>%
          lapply(anchors(ii), overlapsAny, .) %>%
          bind_cols() %>%
          rowSums() %>%
          `==`(2) %>%
          dd$d[.] %>%
          tibble(v = .)
      }) %>%
      bind_rows(.id = 'grp') %>%
      filter(grepl('only|three', grp)) %>%
      mutate(grp = gsub('and', '&', grp) %>%
               gsub('_', ' ', .))
    
    pd <- group_by(vv, grp) %>%
      do(getWhisks(.$v)) %>%
      ungroup() %>%
      mutate(nm = i$nm)
    
    
    ann <- compare_means(v ~ grp, vv)
    
    r <- diff(range(c(pd$ymin, pd$ymax)))
    
    annn <- ann %>%
      filter(group1 == 'All three') %>%
      mutate(y.position = max(pd$upper) + r / 10 * (1:n()),
             nm = i$nm)
    
    yl <- sprintf('%s - %s', names(i$fs)[1], names(i$fs)[2])
    #if (proj == 'mESC') {
    yl <- paste0('\u0394Pairwise interaction\n', yl)
    #}
    ggplot(pd, aes(x = grp, y = middle, group = grp, fill = grp, color = grp)) +
      geom_hline(yintercept = 0, color = 'black') +
      geom_boxplot(aes(ymin = ymin, ymax = ymax, fill = grp,
                       lower = lower, upper = upper,
                       notchupper = notchupper,
                       notchlower = notchlower,
                       middle = middle, color = grp),
                   notch = T, stat = "identity", width = .6,
                   show.legend = F) +
      geom_crossbar(aes(ymin = middle, ymax = middle),
                    color = "white", width = 0.4, fatten = 0) +
      stat_pvalue_manual(annn, inherit.aes = F, label = 'p.signif') +
      facet_wrap(~nm)+
      scale_y_continuous(position = 'right') +
      ylab(yl) +
      scale_fill_manual(values = clrs[c(2,3,1)]) +
      scale_color_manual(values = clrs[c(2,3,1)]) +
      theme(plot.background = element_blank(),
            legend.position = 'none',
            axis.text = element_text(color = 'black', size = 11),
            panel.background = element_blank(),
            strip.background = element_rect(fill = 'black'),
            panel.grid = element_blank(),
            axis.ticks = element_blank(),
            panel.grid.major = element_line(color = 'grey80', linetype = 'dashed'),
            axis.title.x = element_blank(),
            axis.text.x = element_text(angle = 45, hjust = 1),
            strip.text = element_text(color = 'white', size = 11))
  }, ., names(.)) -> pp
ggsave('sf7_c.csight.pdf',pp$hiPSC,height = 5.2, width = 2.5, bg = 'transparent', device = cairo_pdf)

pdf('sf7_c.euler.pdf', height = 2, width = 2)
u <- GRangesList(pks$hiPSC) %>% unlist() %>% reduce() %>% subsetByOverlaps(cgis$hg38)
p <- lapply(pks$hiPSC, function(x) overlapsAny(u, x)) %>% bind_cols()
euler(p, shape = 'ellipse') -> fit
names(fit$fitted.values) %>% tibble(nm = .) %>% 
  mutate(clr = case_when(nm == 'CBX2' ~ clrs[4], nm == 'RING1B' ~ clrs[1],
                         nm == 'H3K27me3' ~ clrs[3], 
                         nm == 'CBX2&RING1B&H3K27me3' ~ clrs[2],
                         T ~ '#dddddd')) %>% 
  pull(clr) %>%
  plot(fit, quantities = T, fills = .)
dev.off()


tclrs <- c('#e45756', '#4c78a8')

fall <- matrix(c(255, 255, 255,
                 255, 255, 204,
                 255, 237, 160,
                 254, 217, 118,
                 254, 178, 76,
                 253, 141, 60,
                 252, 78, 42,
                 227, 26, 28,
                 189, 0, 38,
                 128, 0, 38,
                 0, 0, 0), nrow = 3) %>%
  apply(2, function(a) rgb(a[1], a[2], a[3], maxColorValue = 255))

rot <- function(df, degree) {
  dfr <- df
  degree <- pi * degree / 180
  l <- sqrt(df$x^2 + df$y^2)
  teta <- atan(df$y / df$x)
  dfr$x <- round(l * cos(teta - degree))
  dfr$y <- round(l * sin(teta - degree))
  return(dfr)
}

reg <- GRanges('chr10', IRanges(100.4e6, 101.6e6))

res <- 10000

mats <- c('WT' = 'hiPSC_WT', 'NSD1+/-' = 'hiPSC_NSD1het') %>%
  lapply(function(samp) {
    crd <- sprintf('../data/hic/%s/Full_Mats_Coords/coords_%s_res_%d.npy',
                   samp, seqnames(reg), res) %>%
      npyLoad('integer')
    r <- (which(crd == start(reg) / res) - 100):(which(crd == end(reg) / res) + 100)
    sprintf('../data/hic/%s/Full_Enhanced/%s_res_%d.npy',
            samp, seqnames(reg), res) %>%
      npyLoad() %>% 
      .[r, r] %>%
      as.data.frame() %>% 
      mutate(y = 1:n()) %>% 
      pivot_longer(-y, names_to = 'x', values_to = 'z') %>% 
      mutate(x = sub('^V', '', x) %>% as.integer())
  })

p1 <- mats %>%
  bind_rows(.id = 'samp') %>%
  group_by(samp) %>%
  mutate(across(c(x, y), ~.x/sqrt(2))) %>%
  do(., rot(., 45)) %>%
  ungroup() %>%
  filter(between(y, 0, 60)) %>%
  filter(between(x, 101,301)) %>%
  mutate(samp = factor(samp, c('WT','NSD1+/-'))) %>%
  ggplot(aes(x, y, fill = z)) +
  geom_raster() +
  coord_cartesian(expand = 0) +
  scale_fill_gradientn('Interaction',
                       colours = fall, na.value=fall[1],
                       trans = 'log',limits = c(.05, 1),
                       oob = scales::squish, breaks = c(0.1,.5,1),
                       guide = guide_colorbar(barwidth = .5, barheight = 3.8,
                                              title.hjust = .5,
                                              title.vjust = 0.5,
                                              title.position = 'left')) +
  facet_nested('Hi-C' + samp~'hiPSC')+
  geom_label(aes(label = samp, color = samp), x = -Inf, y = Inf, hjust = 0, vjust = 1,
             data = ~distinct(., samp), label.size = NA, fill = '#ffffff99',
             show.legend = F) +
  scale_color_manual(values = tclrs) +
  scale_y_continuous(breaks = c(25, 50),
                     labels = c('500kb', '1mb')) +
  scale_x_continuous(breaks = c(451,551,651,751),
                     labels = c('110', '120', '130', '140mb')) +
  labs(x = seqnames(reg), y = 'Separation') +
  theme(plot.background = element_blank(),
        panel.background = element_blank(),
        panel.grid = element_blank(),
        axis.text = element_text(color = 'black', size = 11),
        strip.background = element_rect(fill = 'black'),
        axis.text.x = element_blank(),
        legend.background = element_blank(),
        legend.key = element_blank(),
        legend.title = element_text(angle = 90),
        axis.title.x = element_blank(),
        strip.text = element_text(color = 'white', size = 11))



gn <- 'hg38'
cnds = c('WT','NSD1+/-')
nm = ''

bws = list(H3K36me2 = c(WT = "hiPSC_WT_H3K36me2.bw", 
                        "NSD1+/-" = "hiPSC_NSD1het_H3K36me2.bw"),
           H3K27me3 = c(WT = "hiPSC_WT_H3K27me3.bw", 
                        "NSD1+/-" = "hiPSC_NSD1het_H3K27me3.bw"),
           RING1B = c(WT = "hiPSC_WT_RING1B.bw", 
                      "NSD1+/-" = "hiPSC_NSD1het_RING1B.bw"),
           CBX2 = c(WT = "hiPSC_WT_CBX2.bw", 
                    "NSD1+/-" = "hiPSC_NSD1het_CBX2.bw")) %>%
  lapply(function(m) {
    m %>%
      lapply(function(f) {
        import.bw(file.path('../data/bws', f), selection = BigWigSelection(reg))
      })
  })


bins <- tile(reg, 500)[[1]]
seqlevels(bins) <- lapply(unlist(bws, recursive = F), function(x) seqlevels(x)) %>%
  Reduce(intersect, .) 
xs <- mid(bins)
scs <- lapply(bws, function(x) {
  lapply(x, function(y) {
    seqlevels(y) <- seqlevels(bins)
    mcolAsRleList(y, 'score') %>%
      binnedAverage(bins, ., 'score') %>%
      score() %>%
      tibble(score = .) %>%
      mutate(idx = xs)
  }) %>% bind_rows(.id = 'samp')
}) %>% bind_rows(.id = 'mark') %>%
  mutate(samp = factor(samp, names(bws[[1]])),
         mark = factor(mark, names(bws)),
         proj = nm)

cgi <- cgis[[gn]] %>%
  subsetByOverlaps(reg) %>%
  as_tibble() %>%
  dplyr::select(1:3) %>%
  {lapply(bws, function(x) lapply(x, function(y) .) %>% bind_rows(.id = 'samp'))} %>%
  bind_rows(.id = 'mark') %>%
  mutate(samp = factor(samp, cnds),
         mark = factor(mark, names(bws)),
         proj = nm)


scs %>%
  arrange(mark, samp) %>%
  mutate(grp = paste(mark, samp) %>% fct_inorder()) %>%
  split(., .$grp) %>%
  lapply(function(x) {
    m <- max(x$score)
    l <- if(m >= 10) {
      floor(m)
    } else if (m >= 1) {
      floor(m * 10) / 10
    } else if (m >= .1) {
      floor(m * 100) / 100
    } else if (m >= .01) {
      floor(m * 1000) / 1000
    } else if (m >= .01) {
      floor(m * 10000) / 10000
    } else {
      floor(m * 100000) / 100000
    }
    scale_y_continuous(limits = c(0, m),
                       breaks = c(0, l),
                       labels = c('', l),
                       expand = expansion(c(0, .2)))
  }) %>% unname() -> yls

p2 <- ggplot(scs, aes(x = idx, y = score )) +
  geom_line(aes(color = samp), size = .5) +
  geom_area(aes(fill = samp), alpha = .75) +
  geom_label(aes(label = samp, color = samp), x = -Inf, y = Inf, hjust = 0, vjust = 1,
             data = ~distinct(., samp, mark), label.size = NA, fill = '#ffffff99') +
  scale_x_continuous(as.character(seqnames(reg)),
                     limits = c(start(reg), end(reg)),
                     breaks = seq(100e6,102e6,1e6),
                     labels = c('100', '101', '102mb'),
                     expand = expansion(.005)) +
  facet_nested(mark + samp ~ proj, scales = 'free')  +
  facetted_pos_scales(y = yls) +
  scale_color_manual(values = tclrs) +
  scale_fill_manual(values = tclrs) +
  ylab('CPM') +
  scale_y_continuous(expand = expansion(c(0, .2)),
                     breaks = scales::pretty_breaks(2)) +
  coord_cartesian(clip = 'off') +
  theme(legend.position = 'none',
        panel.grid = element_blank(),
        plot.background = element_blank(),
        panel.background = element_blank(),
        axis.text = element_text(color = 'black', size = 11),
        axis.line.y = element_line(color = 'black'),
        strip.text.x = element_blank(),
        strip.background.x = element_blank(),
        strip.background = element_rect(fill = 'black'),
        strip.text = element_text(color = 'white', size = 11))


txs <- list(mm10 = 'GCF_000001635.26_GRCm38.p6_genomic.gff.gz',
            hg38 = 'GCF_000001405.40_GRCh38.p14_genomic.gff.gz') %>%
  lapply(function(x) {
    tx <- import(file.path('..', 'data', 'genes', x))
    rnm <- sub('_genomic.gff.gz', '_assembly_report.txt', x) %>%
      file.path('..', 'data', 'genes', .) %>%
      read_delim(comment = '#', delim = '\t', col_names = F) %>%
      filter(X2 == 'assembled-molecule') %>%
      dplyr::select(X7, X10) %>%
      deframe()
    tx[!is.na(tx$tag) & seqnames(tx) %in% names(rnm)] %>%
      keepSeqlevels(names(rnm)) %>%
      renameSeqlevels(rnm)
  })



gclrs <- c('#f28e2b', '#b07aa1')
subsetByOverlaps(txs$hg38, reg) %>%
  as_tibble() %>%
  filter(type %in% c('exon',  'mRNA')) %>%
  dplyr::select(strand, start, end, type, gene) %>%
  rbind(tibble(strand = c('+', '+', '-', '-') %>% fct_inorder(),
               start = rep(-1, 4),
               end = rep(-1, 4),
               gene = rep(c('a','b'), each = 2),
               type = rep(c('mRNA', 'exon'), 2))) %>%
  mutate(forward = strand == '+',
         position = case_when(strand == '-' ~ end, T ~ start)) %>%
  ggplot(aes(xmin = start, xmax = end, y = strand, color = strand,
             fill = strand, forward = forward)) +
  geom_gene_arrow(color = NA, data = ~subset(., type == 'mRNA')) +
  scale_y_discrete(breaks = c('-', '+'), labels = c('-', '+')) +
  coord_cartesian(xlim = c(start(reg), end(reg))) +
  scale_x_continuous(seqnames(reg),
                     limits = c(start(reg), end(reg)),
                     breaks = seq(100e6,102e6,1e6),
                     labels = c('100', '101', '102mb'),
                     expand = expansion(.005)) +
  scale_fill_manual(values = gclrs) +
  scale_color_manual(values = gclrs) +
  theme(plot.background = element_blank(),
        panel.background = element_blank(),
        panel.grid = element_blank(),
        axis.text = element_text(color = 'black', size = 11),
        axis.line.x = element_line(color = 'black'),
        axis.title.y = element_blank(),
        axis.ticks.y = element_blank(),
        strip.background = element_blank(),
        strip.text = element_blank(),
        legend.position = 'none') -> p3

{wrap_plots(p1,p2 +
              theme(legend.position = 'none',
                    panel.grid = element_blank(),
                    plot.background = element_blank(),
                    panel.background = element_blank(),
                    axis.text = element_text(color = 'black', size = 11),
                    axis.line.y = element_line(color = 'black'),
                    strip.text.x = element_blank(),
                    axis.text.x = element_blank(),
                    axis.title.x = element_blank(),
                    axis.ticks.x = element_blank(),
                    strip.background.x = element_blank(),
                    strip.background = element_rect(fill = 'black'),
                    strip.text = element_text(color = 'white', size = 11)),
            p3,nrow=3, heights = c(1,4,.3)) &
    theme(plot.background = element_blank()) }  %>%
  ggsave('sf7_d.pdf', ., width = 5.4, height = 9)




rs <- list(WT = 'hiPSC_WT_H3K27me3.mat.gz',
           Het = 'hiPSC_NSD1het_H3K27me3.mat.gz') %>%
  lapply(function(x) {
    x <- file.path('../data/aggr/lfc', x)
    r <- fread(x, select = 1:6, skip = 1)
    v <- fread(x, drop = 1:6, skip = 1) %>%
      .[,990:1010] %>%
      rowMeans(na.rm = T)
    r[head(order(v, decreasing = T), 1000),1:3]
  })

rs$union <- unique(rbindlist(rs))
rs$inter <- lapply(rs, unique) %>% {inner_join(.$WT, .$Het)}

aggr <- list.files('../data/aggr/lfc', pattern = '.mat.gz', full.names = T) %>%
  setNames(., sub('.mat.gz', '', basename(.))) %>%
  grep('hiPSC', ., value = T) %>%
  lapply(function(x) {
    r <- fread(x, select = 1:6, skip = 1) %>% mutate(idx = 1:n())
    m <- fread(x, drop = 1:6, skip = 1) %>% as.matrix()
    lapply(rs, function(y) {
      m[unique(merge(r, y)$idx),] %>%
        {tibble(mu = colMeans(., na.rm = T),
                std = colSds(., na.rm = T),
                num = colSums(is.finite(.)))} %>%
        mutate(idx = 1:n())
    })
  })


tclrs <- c('#e45756', '#4c78a8')
aggr[grepl('hiPSC', names(aggr))] %>%
  lapply(bind_rows, .id = 'reg') %>%
  bind_rows(.id = 'track') %>%
  mutate(se = std / sqrt(num)) %>%
  filter(reg == 'union') %>%
  separate('track', c(NA, 'cond', 'mark')) %>%
  mutate(cond = factor(cond, c('WT', 'NSD1het')),
         mark = factor(mark, c('H3K27me3', 'RING1B','CBX2'))) %>%
  ggplot(aes(x = idx, y = mu, color = cond, fill = cond, ymin = mu - se, ymax = mu + se)) +
  geom_ribbon(alpha = .5, color = NA)+
  geom_line() +
  facet_grid(mark ~ 'PRC1 reading of H3K27me3', scales = 'free_y') +
  scale_fill_manual(values = tclrs) +
  scale_color_manual(values = tclrs) +
  coord_cartesian(xlim = c(750, 1250)) +
  scale_x_continuous(breaks = c(750, 1000, 1250),
                     labels = c('-250kb','H3K27me3-\nenriched CGI','+250kb')) +
  scale_y_continuous('ChIP-seq enrichment') +
  theme(axis.text = element_text(size = 11, color = 'black'),
        strip.background = element_rect(fill = 'black'),
        strip.text = element_text(size = 11, color = 'white'),
        panel.background = element_rect(fill = NA, color = 'black', size = 1),
        panel.grid = element_blank(),
        legend.title = element_blank(),
        legend.position = c(1,1),
        legend.justification = c(1,1),
        axis.title.x = element_blank(),
        legend.background = element_blank(),
        legend.key = element_blank(),
        plot.background = element_blank()) -> p5

ggsave('sf7_e.pdf', p5, height = 3.4, width = 3.2, bg = 'transparent')



load('../data/pmtcgi/hiPSC.cts.rda')
cgi <- !grepl('^ENS', bins$name)
                
list(CBX2 = c(Het = "hiPSC_NSD1het_CBX2" ,
              WT = "hiPSC_WT_CBX2"),
     RING1B = c(Het = "hiPSC_NSD1het_RING1B",
                WT =  "hiPSC_WT_RING1B"),
     H3K27me3 = c(Het = "hiPSC_NSD1het_H3K27me3",
                  WT = "hiPSC_WT_H3K27me3")) %>%
  Map(function(x, m) {
    lapply(x, function(y) {
      cts[[y]][cgi] * 1e6 / libsz[y]
    }) %>%
      {log2(.$WT / .$Het)}
  }, ., names(.)) %>%
  bind_cols() %>%
  filter(is.finite(rowSums(.))) %>%
  mutate(qnt = ntile(H3K27me3, n = 10) %>% as.character() %>% fct_inseq()) %>%
  ggplot(aes(x = qnt, y = RING1B, group = qnt, color = qnt, fill = qnt)) +
  geom_hline(yintercept = 0) +
  geom_boxplot(aes(fill = qnt), outlier.color = NA, notch = T) +
  coord_cartesian(ylim = c(-.6, 1)) +
  scale_fill_viridis_d(begin = 0, end = .9) +
  scale_color_viridis_d(begin = 0, end = .9) +
  stat_summary(geom = "crossbar", width=0.15, fatten=0, color="white", 
               fun.data = function(x){ return(c(y=median(x), ymin=median(x), ymax=median(x))) }) +
  labs(y = expression(Delta*'RING1B (WT/NSD1het)'),
       x = expression('Decile of'~Delta*'H3K27me3 (WT/NSD1het)')) +
  facet_wrap(~'Differential polycomb occupancy') +
  theme(plot.background = element_blank(),
        panel.background = element_rect(color= 'black', size = 1, fill = NA),
        panel.grid = element_blank(),
        panel.grid.major = element_line(color = 'grey75', linetype = 'dashed'),
        legend.position = 'none',
        legend.background = element_blank(),
        strip.text = element_text(color = 'white', size = 11),
        strip.background = element_rect(fill = 'black'),
        axis.text = element_text(size = 11, color = 'black')) -> p4
ggsave('sf7_f.pdf', p4, height = 3.4, width = 3.2, bg = 'transparent')


tclrs <- c('#e45756', '#4c78a8')
pd <- fread('../data/quant/HOXD8.ddPCR.tsv') %>%
  pivot_longer(c(WT, `NSD1+/-`), names_to = 'cond', values_to = 'y') %>%
  mutate(`Target protein` = factor(`Target protein`,c('CBX2','CBX4','CBX8','EED','EZH2','SUZ12','H3K27me3','RING1B','IgG control'))) %>%
  arrange(`Target protein`, Antibody) %>%
  mutate(cond = factor(cond, c('WT', 'NSD1+/-')),
         a = case_when(`Target protein` != 'CBX2' ~ NA_character_,
                       T ~ c('CST 18687' = 'ab1',
                             'CST E3N6A' = 'ab2',
                             'Novus NBP247524' = 'ab3')[Antibody]),
         x = case_when(is.na(a) ~ as.character(`Target protein`),
                       T ~sprintf('CBX2 (%s)', a)) %>% 
           fct_inorder(),
         idx = as.integer(x),
         xx = ifelse(cond == 'WT', idx - .15, idx + .15)) 
rcts <- pd %>% distinct(x) %>% mutate(idx = as.integer(x), xmin = idx - .5, xmax = idx+ .5, alt = idx %% 2 == 0)

ggplot(pd, aes(x = xx, y = y)) +
  geom_rect(aes(xmin = xmin, xmax = xmax, ymin = -Inf, ymax = Inf, fill = alt),
            data = rcts, inherit.aes = F) +
  scale_fill_manual(values = c('#ffffff00', '#00000022'), guide = guide_none()) +
  new_scale_fill() +
  geom_col(aes(fill = cond), width = .3) +
  scale_fill_manual(values = tclrs) +
  scale_x_continuous(breaks = unique(pd$idx), labels = unique(pd$x)) +
  coord_cartesian(xlim = range(pd$idx)) +
  scale_y_continuous(expand = expansion(c(0, .05))) +
  facet_wrap(~'ddPCR quantification') +
  labs(x = 'Target protein', y = 'Enrichment at HOXD8\nover nearby intergenic region') +
  theme(axis.text = element_text(size = 11, color = 'black'), 
        axis.text.x = element_text(angle = 45, hjust = 1), 
        plot.background = element_blank(), 
        panel.background = element_rect(fill = NA, color = 'black'),
        panel.grid = element_blank(),
        legend.position = c(.5, 1),
        legend.justification = c(.5, 1),
        legend.title = element_blank(),
        legend.background = element_blank(),
        strip.background = element_rect(fill = 'black'),
        strip.text = element_text(size = 11, color = 'white'),
        legend.key = element_blank(),
        panel.grid.major.y = element_line(color = 'grey80', linetype = 'dashed'),
        legend.text = element_text(size = 11)) -> p
ggsave('sf7_g.pdf', p, height = 3.3, width = 6.6, bg = 'transparent')

